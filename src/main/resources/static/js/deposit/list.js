...
layui.use(['laydate', 'table', 'form'], function () {
  ...
  tableIns = table.render({
    ...
    cols: [[
      ...
      {
        field: 'code', title: '充值码', width: 120, templet: function (d) {
          return d.id ? d.id : '-';
        }
      },
      {
        field: 'totalFee', title: '充值金额', width: 100, templet: function (d) {
          return d.totalFee ? d.totalFee : '-';
        }
      },
      {
        field: 'ctime', title: '充值时间', width: 200, templet: function (d) {
...
function confirm(tableIns, params) {
  ajaxPost($, 'api/deposit/confirm', params,
    function (response) {
      tableIns.reload({
        page: {
          curr: curPage
      ...
      layer.confirm('已转账，确认充值成功', function (index) {
        layer.close(index);
      });
    },
    function (response) {
